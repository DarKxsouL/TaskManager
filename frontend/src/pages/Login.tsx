...
//     try {
//       await login({ email, password, loginAs: systemRole });
...
//   email, 
//   password, 
//   systemRole: signupRole,
//   companyName: signupRole === 'Admin' ? companyName : undefined
// });
...
//                       <div className='w-full border rounded-lg flex gap-2'>
//                       <input type={showPass? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-80 outline-none px-3 py-2 " placeholder="Enter your password" />
//                       <button type='button' onClick={(() => toggleShowPass())} className='hover:cursor-pointer px-2'>{showPass ? <LuEyeClosed size={20} /> : <LuEye size={20} />}</button>
//                     </div>
//                     </div>
...
//                       <button type="button" onClick={() => { setIsFlipped(true); setSystemRole('Employee'); }} className="text-red-500 cursor-pointer font-bold hover:underline">Sign Up</button>
//                     </div>
//                   </form>
//                 </>
//               )}
...
//                   <label className="block text-gray-700">Password</label>
//                   <div className='w-full border rounded-lg flex gap-2'>
//                   <input type={showPass ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-80 outline-none px-3 py-2 " placeholder="Create a password" />
//                   <button type='button' onClick={(() => toggleShowPass())} className='hover:cursor-pointer px-2'>{showPass ? <LuEyeClosed size={20} /> : <LuEye size={20} />}</button>
//                 </div>
//                 </div>
...
//                   <button type="button" onClick={() => { setIsFlipped(false); setSignupRole('Employee'); }} className="text-blue-500 cursor-pointer font-bold hover:underline">Login</button>
//                 </div>
//               </form>
//             </div>
...
function Login() {
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-black/80 relative overflow-hidden">
        ...
        <div className="w-96 h-[600px] [perspective:1000px] z-10">
          ...
          >
            ...
            <div className="absolute inset-0 w-full h-full flex flex-col justify-center items-center bg-white p-8 rounded-xl shadow-white/30 shadow-xl [backface-visibility:hidden]">
              ...
              {isResetting ? (
                <>
                  ...
                  {!otpSent ? (
                    <form onSubmit={handlePasswordResetRequest} className="space-y-4 w-full">
                      <p className="text-slate-600 text-sm text-center">Enter your email to receive a One-Time Password (OTP).</p>
                      <div>
                      ...
                      <button type="submit" disabled={loading} className="w-full text-lg font-bold bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-500 text-white py-2 rounded-xl hover:shadow-lg shadow-md shadow-blue-500/20 disabled:opacity-50 transition-all">
                        {loading ? 'Sending...' : 'Get OTP'}
                      </button>
                    </form>
                  ) : (
                    <form onSubmit={handleFinalReset} className="space-y-4 w-full">
                      <p className="text-slate-600 text-xs text-center">Enter the code sent to <b>{email}</b></p>
                      <div>
                        <label className="block text-slate-700 font-semibold mb-1">OTP Code</label>
                        <input 
                          type="text" 
                          required 
                          value={otp} 
                          onChange={(e) => setOtp(e.target.value)} 
                          className="w-full px-3 py-2 border rounded-xl text-center tracking-widest font-bold focus:outline-none focus:ring-2 focus:ring-green-500" 
                          placeholder="123456" 
                          maxLength={6} 
                        />
                      </div>
                      ...
                      <button type="submit" disabled={loading} className="w-full text-lg font-bold bg-green-500 text-white py-2 rounded-xl hover:bg-green-600 disabled:opacity-50 transition-colors">
                        {loading ? 'Processing...' : 'Change Password'}
                      </button>
                    </form>
                  )}
                  ...
                  <div className="flex justify-center mt-6">
                    <button 
                      type="button" 
                      onClick={() => { setIsResetting(false); setOtpSent(false); }} 
                      className="text-slate-500 cursor-pointer font-bold hover:text-slate-800 text-sm"
                    >
                      ← Back to Login
                    </button>
                  </div>
                ...
                <>
                  ...
                  <div className="w-full mb-4">
                      <label className="block text-slate-700 text-sm font-bold mb-1">Login As</label>
                      <select 
                        value={systemRole} 
                        onChange={(e) => setSystemRole(e.target.value)}
                        className="w-full px-3 py-2 border border-slate-300 rounded-xl outline-none focus:border-blue-500"
                      >
                          <option value="Employee">Employee</option>
                          <option value="Admin">Admin / CEO</option>
                      </select>
                  ...
                  <form onSubmit={handleLogin} className="space-y-4 w-full">
                    <div>
                      ...
                      <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-3 py-2 border rounded-xl" placeholder="Enter your email" />
                    </div>
                    <div>
                      <label className="block text-slate-700">Password</label>
                      <div className='w-full border rounded-xl flex gap-2'>
                      <input type={showPass? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-80 outline-none px-3 py-2 " placeholder="Enter your password" />
                      <button type='button' onClick={(() => toggleShowPass())} className='hover:cursor-pointer px-2'>{showPass ? <LuEyeClosed size={20} /> : <LuEye size={20} />}</button>
                    </div>
                    </div>
                    ...
                    <button type="submit" disabled={loading} className="w-full text-lg font-bold transition-all duration-300 ease-in-out bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-500 text-white py-2 rounded-xl cursor-pointer hover:shadow-lg shadow-md shadow-blue-500/20">
                      {loading ? 'Logging in...' : 'Login'}
                    </button>
                    
                    <div className="text-center text-slate-400 text-sm">------------------- Login with -------------------</div>
            ...
            <div className="absolute inset-0 w-full h-full flex flex-col justify-center items-center bg-white p-8 rounded-xl shadow-lg [backface-visibility:hidden] [transform:rotateY(180deg)]">
              <h2 className="text-2xl font-bold mb-6 text-center">Create Account</h2>
              <div className="w-full mb-4">
                  <label className="block text-slate-700 text-sm font-bold mb-1">Role</label>
                  <select 
                    value={signupRole} 
                    onChange={(e) => setSignupRole(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-xl outline-none focus:border-green-500"
                  >
                      <option value="Employee">Employee</option>
                      <option value="Admin">Admin / CEO</option>
                  </select>
  ...
  <input
    ...
    value={companyName}
    onChange={(e) => setCompanyName(e.target.value)}
    className="w-full border border-slate-300 rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-blue-400"
    required
  />
)}
              ...
              <form onSubmit={handleSignup} className="space-y-4 w-full">
                <div>
                  ...
                  <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-2 border rounded-xl" placeholder="Create a username" />
                </div>
                <div>
                  <label className="block text-slate-700">Email</label>
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-3 py-2 border rounded-xl" placeholder="Enter your email" />
                ...
                <button type="submit" disabled={loading} className="w-full text-lg font-bold transition-colours duration-300 ease-in-out bg-green-500 text-white py-2 rounded-xl cursor-pointer hover:bg-green-600 mt-6">
                    {loading ? 'Creating Account...' : 'Sign Up'}
                </button>
