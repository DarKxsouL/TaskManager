...
function Assigned() {
  ...
  const getFilterButtonStyle = (isActive: boolean, baseClasses: string) => {
    return `${baseClasses} transition-all duration-300 ease-in-out ${isActive ? 'ring-2 ring-blue-500 font-bold shadow-sm' : 'opacity-70 hover:opacity-100'}`;
  }
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/40 to-indigo-50 pt-10 pb-12 px-0 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-white">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm mt-0.5">Tasks that have been assigned to you</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="bg-white/15 backdrop-blur rounded-2xl px-5 py-3 border border-white/20">
                ...
                <div className="text-[11px] uppercase tracking-wider text-blue-100 mt-1">Showing</div>
              </div>
              <div className="bg-white/15 backdrop-blur rounded-2xl px-5 py-3 border border-white/20">
                <div className="text-2xl font-bold leading-none">{assigntasks.length}</div>
                <div className="text-[11px] uppercase tracking-wider text-blue-100 mt-1">Total</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* FILTERS SECTION */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/70 flex p-6 gap-x-20 mb-6">
            <div>
                <div className="py-2 font-semibold text-slate-600 text-sm uppercase tracking-wide">Sort by</div>
                <select 
                  className="w-60 px-3 py-2 rounded-xl border border-slate-200 bg-white outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all"
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value)}
                >
                    <option value="date_asc">DueDate (Ascending)</option>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/70 p-6 overflow-scroll h-2/3">
          {processedTasks.length === 0 ? (
            <div className="text-center mt-20">
              ...
              <h3 className="text-slate-800 text-xl font-bold">No assigned tasks match your filters</h3>
